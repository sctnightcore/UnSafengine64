...
bool read_exports(ADDRINT saddr, vector<ADDRINT> &v_fn_addr, vector<string> &v_fn_name) {
	NW::PIMAGE_DOS_HEADER dos0 = reinterpret_cast<NW::PIMAGE_DOS_HEADER>(saddr);
	NW::PIMAGE_NT_HEADERS nt0 = reinterpret_cast<NW::PIMAGE_NT_HEADERS>(saddr + dos0->e_lfanew);


	ADDRINT img_size = (ADDRINT)(nt0->OptionalHeader.SizeOfImage);
	ADDRINT edir_va = nt0->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress;
	ADDRINT edir_size = (ADDRINT)(nt0->OptionalHeader.DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].Size);
	...
	*fout << "edir size:" << toHex(edir_size) << endl;
	...
	NW::PIMAGE_EXPORT_DIRECTORY edir = reinterpret_cast<NW::PIMAGE_EXPORT_DIRECTORY>(edir_va);
	...
	*fout << "addr_functions:" << toHex(addr_functions) << endl;
	...
	size_t num_functions = (size_t)edir->NumberOfFunctions;
	*fout << "num_functions:" << toHex(num_functions) << endl;
	// fout->flush();

	size_t num_names = (size_t)edir->NumberOfNames;
	*fout << "num_names:" << toHex(num_names) << endl;
	// fout->flush();	

	for (size_t i = 0; i < num_functions; i++) {
		ADDRINT* pfn_addr = reinterpret_cast<ADDRINT*>(addr_functions + i * ADDRSIZE);
	...
	for (size_t i = 0; i < num_names; i++) {
		ADDRINT* pnm_addr = reinterpret_cast<ADDRINT*>(addr_names + i * ADDRSIZE);
		if (*pnm_addr > img_size) {
			*fout << "nm_addr is beyond the image size" << endl;
			return false;
		}
		char* nm_addr = reinterpret_cast<char*>(*pnm_addr + saddr);
