...
KNOB<ADDRINT> KnobOEP(KNOB_MODE_WRITEONCE, "pintool", "oep", "0", "specify OEP manually");
...
{    
    ...
    WW::IMAGE_DOS_HEADER* dos0 = (WW::IMAGE_DOS_HEADER*)loadBase0;
    WW::IMAGE_NT_HEADERS* nt0 = (WW::IMAGE_NT_HEADERS*)(loadBase0 + dos0->e_lfanew);
    WW::IMAGE_SECTION_HEADER* sect0 = (WW::IMAGE_SECTION_HEADER*)
        (loadBase0 + dos0->e_lfanew + sizeof(WW::DWORD) + sizeof(nt0->FileHeader) + nt0->FileHeader.SizeOfOptionalHeader);
...
void* MakeImportSection(UINT32* size, UINT32* idt_size, UINT32 vloc)
{
    ...
    *fout << "# number of dll modules: " << ndll << endl;
    ...
    WW::IMAGE_IMPORT_DESCRIPTOR* iid = (WW::IMAGE_IMPORT_DESCRIPTOR*)(import_sec_buf);
    ...
    *fout << "# Import Directory Table at " << toHex(iid) << endl;
    ...
    memset(&iid[i], 0, sizeof(WW::IMAGE_IMPORT_DESCRIPTOR));  // last import directory entry

    // Make Import Names & IAT
    ADDRINT iin = ilt0 + iltsize;

    iid = (WW::IMAGE_IMPORT_DESCRIPTOR*)(import_sec_buf); // import names
    ilt = ilt0;	// IAT
    ...
    if (is_enable_dump && tid == 0) {
        ...
        while (run_until_api_function_status != RunUntilAPIFunctionStatus::kUninitilaized) {            
            if (run_until_api_function_status == RunUntilAPIFunctionStatus::kCheckNextFunction) {
                ...
                // *out << "# Checking Obfuscated Call " << toHex(obfuscated_call_source) << endl;
