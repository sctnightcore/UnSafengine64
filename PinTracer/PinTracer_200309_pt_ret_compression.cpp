...
KNOB<bool> KnobCount(KNOB_MODE_WRITEONCE, "pintool", "count", "0", "count instructions & basic blocks");
KNOB<bool> KnobGraph(KNOB_MODE_WRITEONCE, "pintool", "graph", "0", "build cfg");
...
KNOB<bool> KnobBlockTracePT(KNOB_MODE_WRITEONCE, "pintool", "bbpt", "", "specify whether to trace basic blocks execution + pt information");
	...
	if (isBlockTracePT) {
		for (BBL bbl = TRACE_BblHead(trc); BBL_Valid(bbl); bbl = BBL_Next(bbl)) {
			...
			if (INS_Category(lastins) == XED_CATEGORY_CALL) {
				ADDRINT nextaddr = lastaddr + INS_Size(lastins);
#if DEBUG_PT_TRACE == 1
				*fout << "call_ins@" << toHex(bbl_addr) << " ret to:" << toHex(nextaddr) << endl;
#endif
				bbl_last_ins_call[bbl_addr] = lastaddr + INS_Size(lastins);
			}

			if (INS_Category(lastins) == XED_CATEGORY_RET) {
#if DEBUG_PT_TRACE == 1
				*fout << "ret_ins@" << toHex(bbl_addr) << endl;
#endif
				bbl_last_ins_ret.insert(bbl_addr);
			}

			if (INS_Category(lastins) == XED_CATEGORY_COND_BR) {
			...
			if (IS_MAIN_IMG(bbl_addr) && get_mwblock(bbl_addr) && get_meblock(addr) == 0) {
...
{	
	...
	*fout << "B " << tid << ' ' << toHex(addr) << ' ' << toHex(size) << ' ' << GetAddrInfo(addr) << endl;	
	// *fout << toHex(addr) << ' ' << tid << ' ' << toHex(size) << endl;
...
// process PT call/ret
vector<ADDRINT> pt_call_stk;


void BBL_PT_CodeExecuted(ADDRINT addr, ADDRINT lastaddr, THREADID tid)
{
	ADDRINT prevaddr, nextaddr;
	string tnt_str;
	// emulate 
	// bool gen_tip_pkt = true;
	...
	*fout << "B " << toHex(addr) << endl;
	...
	// Check pt call/ret
	// If the last instruction of the previous basic block is call instruction, 
	// Push the return address to the own stack. 
	// If return instruction is executed, 
	// check whether the popped address is the same as the return address. 
	// If the popped address is the same as the return address, 
	// no tip packet will be generated.
	nextaddr = bbl_last_ins_call[prevaddr];
	if (nextaddr != 0) {		
		pt_call_stk.push_back(nextaddr);
#if DEBUG_PT_TRACE == 1
		* fout << toHex(prevaddr) << " call " << toHex(addr) << ' ' << toHex(nextaddr) << endl;
		*fout << "Stack: ";
		for (auto it : pt_call_stk) {
			*fout << hex << it << ' ';
		}
		*fout << endl;
#endif
	...
	if (bbl_last_ins_ret.count(prevaddr)) {
		if (!pt_call_stk.empty()) {		
#if DEBUG_PT_TRACE == 1
			* fout << "ret -> " << toHex(addr) << endl;
			*fout << "Stack: ";
			for (auto it : pt_call_stk) {
				*fout << hex << it << ' ';
			}
			*fout << endl;
			...
			ADDRINT call_ret_addr = pt_call_stk.back();
			pt_call_stk.pop_back();
			if (call_ret_addr == addr) {
				gen_tip_pkt = false;
#if DEBUG_PT_TRACE == 1
				*fout << "not gen tip" << endl;
#endif
			}
		}
	}
	...
	set_meblock(addr);
	//string disasm_line;
	//get_disasm(lastaddr, disasm_line);
	
	

	if (bbl_has_indirect_br_tgt.count(prevaddr) && gen_tip_pkt) {
		PRINT_TNT_STR();
		PIN_GetLock(&lock, tid + 1);
		*fout << "tip " << toHex(addr) << endl;
		PIN_ReleaseLock(&lock);
	...
	// *fout << "B " << tid << ' ' << toHex(addr) << ' ' << disasm_line << endl;
	//PIN_GetLock(&lock, tid + 1);
	//*fout << "B " << toHex(addr) << endl;
	//PIN_ReleaseLock(&lock);
}
...
void INS_Skip_ExeptionHandler(CONTEXT* ctxt, ADDRINT addr, ADDRINT toaddr, THREADID tid)
{	
	ADDRINT rsp, stktop;
	UINT8 buf[8];
	bool tf;
...
void INS_Memtrace_MR(CONTEXT* ctxt, ADDRINT ip, size_t mSize, ADDRINT targetAddr, THREADID threadid)
{
	...
	*fout << "R " << threadid << ' ' << toHex0(ip) << ' ' << toHex0(targetAddr) << ' ' << StringHex(mem_value, mSize * 2, false) << endl;;
}
...
void INS_Memtrace_MW_before(CONTEXT* ctxt, ADDRINT ip, size_t mSize, ADDRINT targetAddr, THREADID threadid)
{
	...
	delayed_msg << "W " << threadid << ' ' << toHex0(ip) << ' ' << toHex0(targetAddr) << ' ' << mSize;
...
int main(int argc, char *argv[])
{
	...
	if (outputFileName == "result.txt")
	{
		...
		if (isGraph) {
			dot_outputFileName = outputFileName + ".dot";
		}
		outputFileName += ".txt";	
	}
	LOG(outputFileName);
	...
	isCount = KnobCount.Value();
	isGraph = KnobGraph.Value();
	...
	isBlockTracePT = KnobBlockTracePT.Value();
